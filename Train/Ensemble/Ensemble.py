...
CALENDAR_ONLY = ["dia_da_safra"]

SHARED = ["ocorrencia_latitude", "ocorrencia_longitude", "enso"]
...
class EnsemblePredictor:
    """
    Wraps the two base models and the meta-model behind a single
    predict(), so the rest of the pipeline treats it like any estimator.
    """

    def __init__(
        self, weather_model, calendar_model, meta_model, weather_cols, calendar_cols
    ):
    ...
    def predict(self, X):
        weather_pred = self.weather_model.predict(X[self.weather_cols])
        calendar_pred = self.calendar_model.predict(X[self.calendar_cols])

        # The meta-model decides how much each specialist is worth.
        stacked = np.column_stack((weather_pred, calendar_pred))

        return self.meta_model.predict(stacked)

    @property
    def feature_names_in_(self):
        extra = [c for c in self.calendar_cols if c not in self.weather_cols]
        return np.array(list(self.weather_cols) + extra)

    @property
    def feature_importances_(self):
        # Only the weather model's importances are meaningful: the
        # calendar model has a handful of features and the meta-model
        # has just two.
        return self.weather_model.feature_importances_
...
def split_feature_sets(columns):
    """
    Split the feature columns between the two specialists.
    """
    missing = [c for c in CALENDAR_ONLY if c not in columns]
    if missing:
        raise ValueError(f"Calendar features missing from X: {missing}")

    calendar_cols = [c for c in CALENDAR_ONLY if c in columns]
    calendar_cols += [c for c in SHARED if c in columns]

    weather_cols = [c for c in columns if c not in CALENDAR_ONLY]

    return weather_cols, calendar_cols
...
def kfold_train(
    ...
    """
    Train both specialists inside the same cross-validation loop and fit the
    meta-model on their out-of-fold predictions, then return the metrics and
    the final fitted ensemble.
    """

    sgkf = StratifiedGroupKFold(n_splits=n_splits, shuffle=True, random_state=42)

    weather_cols, calendar_cols = split_feature_sets(X.columns)
    report_file.write(f"Weather features ({len(weather_cols)}): {weather_cols}\n")
    report_file.write(f"Calendar features ({len(calendar_cols)}): {calendar_cols}\n")

    r2s, maes, rmses = [], [], []

    # Every row gets one prediction from each specialist, made by a model
    # that never saw it.
    oof_weather = np.zeros(len(X))
    oof_calendar = np.zeros(len(X))
    ...
    for fold, (train_idx, val_idx) in enumerate(sgkf.split(X, y, groups=groups), 1):
        ...
        weather_model = create_model()
        weather_model.fit(X_train[weather_cols], y_train)
        oof_weather[val_idx] = weather_model.predict(X_val[weather_cols])

        calendar_model = create_model()
        calendar_model.fit(X_train[calendar_cols], y_train)
        oof_calendar[val_idx] = calendar_model.predict(X_val[calendar_cols])

    # The meta-model learns how much weight each specialist deserves.
    stacked = np.column_stack((oof_weather, oof_calendar))

    meta_model = Ridge(alpha=1.0)
    meta_model.fit(stacked, y)

    weather_weight, calendar_weight = meta_model.coef_
    report_file.write(
        f"\nEnsemble weights - weather: {weather_weight:.4f} | "
        f"calendar: {calendar_weight:.4f}\n"
    )

    oof_true = y.to_numpy()
    oof_pred = meta_model.predict(stacked)

    r2s.append(r2_score(oof_true, oof_pred))
    maes.append(mean_absolute_error(oof_true, oof_pred))
    rmses.append(np.sqrt(mean_squared_error(oof_true, oof_pred)))

    if fixed_threshold is not None:
        best_threshold = fixed_threshold
        report_file.write(f"Fixed threshold: {best_threshold:.4f}\n")
    else:
    ...
    # Train the final models with all the data
    final_weather = create_model()
    final_weather.fit(X[weather_cols], y)

    final_calendar = create_model()
    final_calendar.fit(X[calendar_cols], y)

    final_model = EnsemblePredictor(
        final_weather, final_calendar, meta_model, weather_cols, calendar_cols
    )

    feature_importance = calculate_feature_importance(
        final_weather, X[weather_cols], report_file
    )
    ...
    return {
        ...
        "calendar_weight": float(calendar_weight),
...
def run(
    ...
    harvests: list = None,
    model_type: str = "xgb",
    model_name: str = "Ensemble_weather_calendar",
    ...
    cut = "fixo" if fixed_threshold is not None else f"beta{beta}"
    run_id = (
        f"{execution_started_at:%Y%m%d_%H%M%S}_{model_type}"
        f"_{cut}_bal{int(balanced)}_temp{int(temp_includes)}"
    )
    run_dir = os.path.join("Train/Ensemble/Runs", run_id)
    ...
    with open(output_report_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"Model: {model_type} | cut: {cut} | balanced: {balanced} | "
            f"temp_includes: {temp_includes}\n"
        )
            ...
            results.append(
                {
                    ...
                    "weather_weight": metrics_kfold["weather_weight"],
                    "calendar_weight": metrics_kfold["calendar_weight"],
                    "TP": metrics_test["tp"],
                    "FP": metrics_test["fp"],
                    "FN": metrics_test["fn"],
                    "TN": metrics_test["tn"],
    ...
    append_to_index(run_id, df_results, model_type, cut, beta, balanced, temp_includes)
    print(f"\nRun saved to {run_dir}")

    return df_results


def append_to_index(run_id, df_results, model_type, cut, beta, balanced, temp_includes):
    """
    ...
    index_path = "Train/Ensemble/runs_index.csv"
    ...
    row = {
        ...
        "mean_weather_weight": df_results["weather_weight"].mean(),
        "mean_calendar_weight": df_results["calendar_weight"].mean(),
    }
...
def write_summary(df_results, output_file):
    ...
    output_file.write(
        "Ensemble weights - weather: {:.4f} | calendar: {:.4f}\n".format(
            df_results["weather_weight"].mean(),
            df_results["calendar_weight"].mean(),
        )
    )


# The cut used when the threshold is not calibrated on training
FIXED_THRESHOLD = 0.50


if __name__ == "__main__":
    for balanced in (False, True):
        for beta, fixed in ((1.0, None), (2.0, None), (1.0, FIXED_THRESHOLD)):
            cut = "fixo" if fixed is not None else f"beta{beta}"
            print(f"\n=== balanced={balanced} cut={cut} ===")
            run(
                datetime.now(),
                beta=beta,
                balanced=balanced,
